...
# ==========================================================
# Adaptive OFDM Bit Loading – PHYSICALLY CORRECT MODEL
# ==========================================================
...
# Constellations
# -------------------------------
def get_constellation(Qm):
    if Qm == 1:
        bits = torch.tensor([[0], [1]])
        const = torch.tensor([-1+0j, 1+0j])

    elif Qm == 2:
        bits = torch.tensor([[0,0],[0,1],[1,0],[1,1]])
        const = torch.tensor([-1-1j, -1+1j, 1-1j, 1+1j]) / np.sqrt(2)
...
# -------------------------------
# BER-Constrained Bit Loading
# -------------------------------
def bit_loading_ber_constrained(sinr_db, ber_ref):
    bps = torch.zeros_like(sinr_db, dtype=torch.int)
    bps[sinr_db >= 5]  = 1
    bps[sinr_db >= 10] = 2
...
# -------------------------------
# SDR-Faithful Channel Simulation
# -------------------------------
def simulate_bitloaded_ofdm(
    H_estim,
    noise_power,
    bits_per_sc,
    sinr_sc_db,
    ...
    for _ in range(n_frames):
        for k in range(F):
            ...
            Qm = bits_per_sc[k].item()
            if Qm == 0:
                continue

            bits_ref, const = get_constellation(Qm)

            idx = torch.randint(0, const.shape[0], (1,))
            tx_bits = bits_ref[idx]
            tx_sym  = const[idx]

            # -------------------------------
            # Time-varying effective channel
            # -------------------------------
            amp_jitter   = 0.12 * torch.randn(1)
            phase_jitter = 0.08 * torch.randn(1)

            h_eff = H_estim[k] * (1 + amp_jitter) \
                    * torch.exp(1j * phase_jitter)

            # -------------------------------
            # Noise (ZF noise enhancement)
            # -------------------------------
            noise = (torch.randn(1) + 1j*torch.randn(1)) \
                    * torch.sqrt(noise_power / 2)

            rx = h_eff * tx_sym + noise

            # ZF equalization (realistic)
            rx_eq = rx / (H_estim[k] + 1e-12)

            # Residual phase noise
            rx_eq *= torch.exp(1j * 0.05 * torch.randn(1))

            # ML detection
            idx_hat = torch.argmin(torch.abs(rx_eq - const))
            rx_bits = bits_ref[idx_hat]

            bit_err += torch.sum(rx_bits != tx_bits).item()
            bit_tot += Qm
...
# ======================================================
# Inputs from SDR Run-1
# ======================================================
# H_estim      : [F] complex
# noise_power  : scalar
# BER_run1     : measured BER

sinr_sc_db = 10 * torch.log10(torch.abs(H_estim)**2 / noise_power)

bits_per_sc = bit_loading_ber_constrained(sinr_sc_db,BER)

BER_sim = simulate_bitloaded_ofdm(
    H_estim,
    noise_power,
    bits_per_sc,
    sinr_sc_db
...
# Throughput
# -------------------------------
bits_per_ofdm = bits_per_sc.sum().item()
bits_per_tti  = bits_per_ofdm * S
data_rate     = bits_per_tti / TTI_duration / 1e6

# -------------------------------
# Results
# -------------------------------
print(f"Run-1 BER        : {BER:.4e}")
print(f"Bit-loaded BER          : {BER_sim:.4e}")
print(f"Bits per OFDM symbol    : {bits_per_ofdm}")
print(f"Bits per TTI            : {bits_per_tti}")
