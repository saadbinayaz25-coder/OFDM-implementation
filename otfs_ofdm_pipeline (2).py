...
use_sdr           = False   # set True to use real SDR hardware
...
class OTFSModulator:
    ...
    def channel_estimate_DD(self, Y_dd, pilot_power=1.0, plot=False):
        ...
        # Noise floor from bins well outside the guard zone
        noise_ref = []
        for k in range(N):
            for l in range(M):
        ...
        # Scan the observation window (delay taps are causal: l >= lp)
        for k in range(kp - pg_k, kp + pg_k + 1):
            for l in range(lp, lp + pg_l + 1):
                kk = k % N;  ll = l % M
                h_val = Y_dd[kk, ll] / pilot_power   # LS estimate
                H_dd[kk, ll] = h_val
                if torch.abs(h_val).item() > detect_thr:
                    tap_list.append((k - kp, l - lp, h_val))

        # Always keep the direct-path tap
        if len(tap_list) == 0:
            h_direct = Y_dd[kp, lp] / pilot_power
            H_dd[kp, lp] = h_direct
            tap_list.append((0, 0, h_direct))
    ...
    def equalize_DD(self, Y_dd, H_dd, noise_power=1e-4):
        """
        MMSE single-tap equalisation in the delay-Doppler domain.

        For DD bins where H_dd ≈ 0 (outside the estimated tap region) we
        still apply a MMSE weight — effectively noise suppression — rather
        than a divide-by-zero that blows up the noise.

        MMSE weight per bin:  w[k,l] = H*[k,l] / (|H[k,l]|² + σ²)

        For bins with |H| ≈ 0 and σ² > 0 this gracefully collapses to ≈ 0,
        which is correct (we have no information there).
        """
        H_conj  = torch.conj(H_dd)
        H_pow   = torch.abs(H_dd) ** 2

        # For the data region (non-pilot/guard), H_dd is 0 — use a
        # flat unit-gain estimate so data symbols are not zeroed out.
        # Strategy: wherever H_pow < noise_power (i.e. we have no tap
        # estimate), substitute H=1 so the MMSE weight becomes 1/(1+σ²).
        H_pow_eff  = torch.where(H_pow < noise_power,
                                 torch.ones_like(H_pow),
                                 H_pow)
        H_conj_eff = torch.where(H_pow < noise_power,
                                 torch.ones_like(H_conj),
                                 H_conj)

        X_dd_eq = Y_dd * H_conj_eff / (H_pow_eff + noise_power)
        return X_dd_eq
    ...
    def demodulate(self, S_tf_rx, noise_power=1e-4, pilot_power=1.0,
        ...
        # 1. SFFT → delay-Doppler domain
        Y_dd = self.SFFT(S_tf_rx)

        # 2. DD-domain channel estimation from the pilot region
        H_dd, taps = self.channel_estimate_DD(
            Y_dd, pilot_power=pilot_power, plot=plot)

        # 3. MMSE equalisation (handles zero-H bins safely)
        X_dd_eq = self.equalize_DD(Y_dd, H_dd, noise_power=noise_power)

        # 4. Rebuild the same data mask used at TX
        data_mask = torch.ones(N, M, dtype=torch.bool)
        for k in range(kp - pg_k - 1, kp + pg_k + 2):
            for l in range(lp - pg_l - 1, lp + pg_l + 2):
                data_mask[k % N, l % M] = False
        ...
        if plot:
            ...
            axes[0].axhline(kp, color='white', lw=0.5, ls='--')
            axes[0].axvline(lp, color='white', lw=0.5, ls='--')
...
# Guard sizes must satisfy:
#   pgd ≥ max_delay_spread (in samples, = max_delay_spread here = 3)
#   pgk ≥ ceil(N * f_D_max / SCS)
#       f_D_max = velocity/c * fc = 30/3e8 * 2.4e9 ≈ 240 Hz
#       ceil(14 * 240 / 15000) = ceil(0.224) = 1  → use 3 for margin
...
def create_OFDM_data(filename=None):
    ...
    if not USE_OTFS:
        ...
        S_tf, X_dd, data_mask = otfs.modulate(
            pdsch_symbols, pilot_power=1.0, plot=True)
...
# ---- Generate TX samples ----
pdsch_bits, pdsch_bits_encoded, pdsch_symbols, TX_Samples = \
...
def sync_iq(tx_signal, rx_signal, leading_zeros, threshold=6, plot=False):
    ...
    if plot:
        ...
        idx_off = range(-10, 50)
        corr_v  = correlation[plot_i-10:plot_i+50]
        disp_v  = [v if v > thr else 0 for v in corr_v]
        plt.figure(figsize=(8, 3))
        plt.bar(idx_off, disp_v); plt.grid()
