...
def create_payload(OFDM_mask, Qm, mapping_table, power=1.0, filename=None):
    """
    Generate data symbols for an OFDM system, either from a binary file or random bits.

    Args:
        OFDM_mask (torch.Tensor): The OFDM block mask indicating payload positions.
        ...
        filename (str, optional): Path to a binary file. If None, random bits are generated.

    Returns:
        tuple: (payload_bits, payload_symbols)
    """
    ...
    if filename is not None:
        with open(filename, "rb") as f:
            data = f.read()
        file_bits = np.unpackbits(np.frombuffer(data, dtype=np.uint8))
        ...
        payload_bits = torch.tensor(file_bits[:num_bits], dtype=torch.float32)
    else:
        # Fall back to random bits
        payload_bits = torch.randint(0, 2, (num_bits,), dtype=torch.float32)
...
def create_OFDM_data():
        # Example call with file
        payload_bits, payload_symbols = create_payload(
        OFDM_mask, Qm, mapping_table, power=1.0, filename="input.bin"
      )

     # Example call with random fallback
        payload_bits, payload_symbols = create_payload(
        OFDM_mask, Qm, mapping_table, power=1.0
    )
...
# =============================================================
# FULL ADAPTIVE OFDM MODULE (CORRECTED)
# =============================================================
...
# =============================================================
# WATERFILLING (PURE NUMPY – NO TORCH MIXING)
# =============================================================

def waterfilling(H, P_total, noise2):

    if torch.is_tensor(H):
        H = H.detach().cpu().numpy()

    if torch.is_tensor(noise2):
        noise2 = float(noise2.detach().cpu().item())
    ...
    eps = 1e-12
    inv_gain = noise2 / (H_abs2 + eps)

    sorted_idx = np.argsort(inv_gain)
    inv_sorted = inv_gain[sorted_idx]

    cumulative = 0.0

    for k in range(1, N + 1):
        cumulative += inv_sorted[k-1]
        water_level = (P_total + cumulative) / k

        if k == N or water_level <= inv_sorted[k]:
            break

    P_sorted = np.maximum(water_level - inv_sorted, 0.0)

    total_power = np.sum(P_sorted)
    if total_power > 0:
        P_sorted *= P_total / total_power

    P_alloc = np.zeros(N)
    P_alloc[sorted_idx] = P_sorted

    capacity = np.sum(np.log2(1 + (P_alloc * H_abs2) / noise2))

    return P_alloc, capacity
...
P_total = 1.0
P_alloc, capacity = waterfilling(H_estim, P_total, noise_power)

print("\nWaterfilling Capacity =", capacity, "bits/OFDM symbol")
# =============================================================
# SNR PER SUBCARRIER
# =============================================================

def compute_snr(H_estim, noise_power, P_alloc):

    P_alloc_t = torch.tensor(P_alloc, dtype=torch.float32)

    snr_lin = P_alloc_t * torch.abs(H_estim)**2 / noise_power
    snr_db = 10 * torch.log10(snr_lin + 1e-12)

    return snr_lin, snr_db


snr_lin_k, snr_db_k = compute_snr(H_estim, noise_power, P_alloc)
...
def bit_loading(snr_db):

    bits = torch.zeros_like(snr_db)
    mods = []
...
bits_per_sc, modulation_map = bit_loading(snr_db_k)
...
def map_bits(bits, mod):

    if mod == "BPSK":
        return torch.tensor((1-2*bits[0]), dtype=torch.complex64)

    if mod == "QPSK":
        b0,b1 = bits
        return torch.tensor(((1-2*b0)+(1j*(1-2*b1)))/np.sqrt(2),
    ...
    if mod == "16QAM":
        ...
        return torch.tensor((re+1j*im)/np.sqrt(10),
                            dtype=torch.complex64)

    if mod == "64QAM":
        b = bits

        re = (1-2*b[0])(4-(1-2*b[2])(2-(1-2*b[4])))
        im = (1-2*b[1])(4-(1-2*b[3])(2-(1-2*b[5])))

        return torch.tensor((re+1j*im)/np.sqrt(42),
                            dtype=torch.complex64)

    return torch.tensor(0+0j, dtype=torch.complex64)


def adaptive_mapper(tx_bits, modulation_map):

    symbols = []
    bit_ptr = 0

    for mod in modulation_map:

        if mod == "NULL":
            symbols.append(torch.tensor(0+0j))
            continue

        nbits = {"BPSK":1,"QPSK":2,"16QAM":4,"64QAM":6}[mod]

        bits = tx_bits[bit_ptr:bit_ptr+nbits]
        bit_ptr += nbits

        symbols.append(map_bits(bits, mod))

    return torch.stack(symbols)
...
total_bits = int(torch.sum(bits_per_sc))
pdsch_bits = pdsch_bits[:total_bits]

mapped_symbols = adaptive_mapper(pdsch_bits, modulation_map)
...
def compute_rate(bits_per_sc):

    bits_symbol = torch.sum(bits_per_sc)

    T_symbol = (FFT_size + CP) / SampleRate
    symbol_rate = 1 / T_symbol

    rate = bits_symbol * symbol_rate

    return bits_symbol.item(), rate.item(), symbol_rate


bits_sym, data_rate, symbol_rate = compute_rate(bits_per_sc)

print("\nBits per OFDM symbol =", bits_sym)
print("Adaptive Data Rate =", data_rate/1e6, "Mbps")


# =============================================================
# BER AFTER ADAPTIVE DEMAPPING
# =============================================================

def demap(sym, mod):

    if mod == "BPSK":
        return torch.tensor([int(sym.real < 0)])
...
def ber_adaptive(equalized, modulation_map):

    rx_bits = []

    mask_payload = OFDM_mask[:, FFT_offset_RX:FFT_offset_RX+F] == 1
    symbols = equalized[mask_payload]

    for k, mod in enumerate(modulation_map):
    ...
    rx_bits = torch.cat(rx_bits)
    tx_bits_used = pdsch_bits[:len(rx_bits)]

    return torch.mean((rx_bits != tx_bits_used).float()).item()


BER_after = ber_adaptive(equalized_H_estim, modulation_map)

print("BER after adaptive BL =", BER_after)
...
rate_fixed = 4 * F * symbol_rate

print("Fixed 16QAM rate =", rate_fixed/1e6, "Mbps")
...
plt.figure()
plt.stem(bits_per_sc.cpu().numpy())
plt.title("Bit Loading")
plt.grid(True)
plt.show()


plt.figure()
plt.bar(np.arange(len(P_alloc)), P_alloc)
